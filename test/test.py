...
import os
import sys
sys.path.append(os.path.abspath(os.path.dirname('../lambda/lambda_function.py')))
...
if __name__ == '__main__':
    ...
    Workloads = [
        {"Vendor":"AWS", "InstanceType":"t3.medium", "Region":"ap-noertheast-2", "AZ":"ap-northeast-2a"},
        {"Vendor":"AWS", "InstanceType":"m6g.large", "Region":"ap-noertheast-2", "AZ":"ap-northeast-2a"}
    ]
    mig_to = [["t3.medium", "c5.large"], ["m6g.large", "c6g.large"]]
    ...
    for idx in range(len(Workloads)):
        ...
        try:
            ...
            result = f"Attempt {i+1}) INIT Fail\n" 
            s3.put_object(Bucket='jupyter-migration', Key=f'TestLog/{today.year}-{today.month}-{today.day}/workload{idx+1}/{i+1}.log', Body=result)
            print(result)
            continue
